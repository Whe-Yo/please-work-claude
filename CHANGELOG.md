...
## [0.3.5] - 260702
### Fixed
- **guard-git 개행 우회 회귀(0.3.4 셀프리뷰 적발)**: 근접성 매칭이 줄 단위 grep과 갈라져 연속행 force push(`git push \` + 개행 + `--force`)를 놓침 — 0.3.4 이전엔 잡히던 케이스. 스캔 전 정규화(백슬래시-개행→공백, 맨 개행→`;` — 셸 의미 보존, bash 3.2 치환만)로 복원. 9케이스 배터리 통과.
