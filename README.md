...
- **강제 층 ([.claude/](.claude/))** — `settings.json` + PreToolUse/SessionStart/**Stop** 훅. §9(.env·파괴적 git)을 `exit 2`로 하드 차단, §7(boost)을 세션 시작 시 환기, **§6(antithesis)을 주요 작업 후 미실행 시 Stop이 턴 종료를 막고 환기**. `setup`이 설치.
